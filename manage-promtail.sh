...
# 帮助信息
usage() {
    echo -e "${YELLOW}Promtail 日志管理脚本${NC}"
    echo -e "${GREEN}Promtail 是一个日志采集代理，用于收集日志并发送到星相平台loki模块中${NC}"
    echo
    echo "用法: $0 [选项]"
    echo "选项:"
    echo "  install    - 安装或更新 Promtail"
    echo "  uninstall  - 卸载 Promtail"
    echo "  status     - 查看 Promtail 运行状态"
...
check_config() {
    ...
    if [ -f "$HOSTS_FILE" ]; then
        ...
        while true; do
            echo -e "\n请选择操作："
            read -p "编辑主机列表[e/E],继续执行[y/Y]，取消执行[n/N]: " choice
            case "$choice" in
                [eE])
                    ...
                    ;;
                [yY])
                    # 确保promtail_machine组存在
                    if ! grep -q "\[promtail_machine\]" "$HOSTS_FILE"; then
                        echo -e "\n[promtail_machine]" >> "$HOSTS_FILE"
                        echo -e "${YELLOW}已添加[promtail_machine]组${NC}"
                    fi
                    return 0
                    ;;
                [nN])
        ...
        echo "[promtail_machine]" > "$HOSTS_FILE"
        ${EDITOR:-vi} "$HOSTS_FILE"
        echo -e "\n${YELLOW}hosts文件内容:${NC}"
        cat "$HOSTS_FILE"
        while true; do
...
parse_ansible_status() {
    ...
    total_hosts=$(awk '/^\[promtail_machine\]/{flag=1;next}/^\[/{flag=0}flag&&/^[0-9]/{count++}END{print count}' "$HOSTS_FILE")
    ...
    if [ "$operation_type" = "uninstall" ]; then
        ...
        if [ "$failed_hosts" -gt 0 ]; then
            ...
            echo -e "${YELLOW}请检查失败主机的具体错误信息，解决方法参考建议：${NC}"
            echo "1. 检查机器是否关闭防火墙"
            echo "2. 检查是否关闭selinux"
            echo "3. 若仍无法部署，请参考手动部署文档进行promtail的手动部署"
        else
...
run_playbook() {
    ...
    ansible-playbook -i "$HOSTS_FILE" playbook/promtail.yml --tags "$tags" | tee "$ansible_output_file"
...
main() {
    ...
    case "$1" in
        install)
            check_config
            run_playbook "install" "安装 Promtail"
            run_playbook "status" "检查 Promtail 状态"
            ;;
        uninstall)
            ...
            read -p "确定要卸载 Promtail 吗？(y/Y): " choice
            case "$choice" in
                [yY])
                    run_playbook "uninstall" "卸载 Promtail"
                    ;;
                *)
                    echo "卸载已取消。"
                    ;;
